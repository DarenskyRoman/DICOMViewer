...
def _instanceNumberscheck(serie):
    """_instanceNumberscheck(serie)
    Checking state of Instance Number tags in serie._datasets.
    If it set to None, serie can't be sorted.
    """

    for k in serie._datasets:
        if k.InstanceNumber == None:
            return False
...
def _splitSerieIfRequired(serie, series):
    ...
    #Checking sortability of serie
    if _instanceNumberscheck(serie):
...
class DicomSeries(object):
    ...
    def description(self):
        ...
        if 'BodyPartExamined' in info:
            fields.append("'" + info.BodyPartExamined + "'")
